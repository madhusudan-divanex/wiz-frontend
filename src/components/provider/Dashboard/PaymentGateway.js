...
function ProviderPaymentGateway() {
    ...
    useEffect(() => {
        console.log(userId, serviceData)
        if (!userId || !serviceData) {
            navigate(-1)
        } else {
            setMemberShipType(serviceData.type)
            setPaymentData({
                ...paymentData, disputeId: serviceData.disputeId, requestId: serviceData.requestId,
                addOnPrice: serviceData?.addOnPrice, addOnName: serviceData?.addOnName, price: serviceData?.price
            })
        }
    }, [userId])
    ...
    async function handlePay() {
        ...
        try {
            if (paymentData.requestId) {
                const result = await securePostData('api/users/request-payment', paymentData)
                if (result.success) {
                ...
                const result = await securePostData('api/users/dispute-payment', paymentData)
                if (result.success) {
                    const modalElement = document.getElementById('exampleModal');
                    const bsModal = new Modal(modalElement); // 👈 Create a Bootstrap modal instance
                    bsModal.show();
                                ...
                                <div className="modal-content text-center z-3">
                                    <div className="modal-body pay-shape py-5">
                                        ...
                                        <div className="" id="popupBox">
                                            <img src='/assets/images/Envelope check 1.png' className='mb-3' style={{width:'15%'}}/>
                                            <h3>Thank You! </h3>
                                            ...
                                            <p>
                                                A member of the Wizbizla team will contact you shortly.
                                            </p>
...
export default ProviderPaymentGateway
